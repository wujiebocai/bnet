...
public:
    ...
    writer(http::header<isRequest, Fields> const& h, value_type const& b);
...
writer(http::header<isRequest, Fields> const& h, value_type const& b)
	: body_(b)
	, text_writer_(const_cast<http::header<isRequest, Fields>&>(h), const_cast<value_type&>(b).text())
	, file_writer_(const_cast<http::header<isRequest, Fields>&>(h), const_cast<value_type&>(b).file())
{
}
