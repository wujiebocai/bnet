...
///////////////////////////kcp stream//////////////////////////////////////////////////////////////////////////
	...
	public:
		...
		inline error_code kcp_handle_recv_t(SessionPtr&& ptr, std::string_view data) {
			...
			if (!ptr->is_started()) return ec_ignore;
	...
	protected:
		...
		asio::awaitable<void> kcp_start_t(SessionPtr&& ptr, std::uint32_t conv) {
			...
			if (!kcp_timer_) kcp_timer_ = std::make_shared<asio::steady_timer>(executor);
            asio::co_spawn(executor, [this, ptr]() { 
				return this->kcp_timer_t(ptr); 
			}, asio::detached);
		...
		void kcp_stop_t() {
			...
			if (kcp_timer_) this->kcp_timer_->cancel();
		}
		...
		template<class SessionPtr>
		asio::awaitable<void> kcp_timer_t(SessionPtr&& ptr) {
			do {
				...
				ptr->kcp_timer_->expires_after(std::chrono::milliseconds(clock2 - clock1));
				asio::error_code ec;
          		co_await ptr->kcp_timer_->async_wait(asio::redirect_error(asio::use_awaitable, ec));
				...
				if (this->kcp_->state == (kcp::IUINT32)-1) {
					if (ptr->state() == estate::started) co_await ptr->co_stop(asio::error::network_reset);
					co_return;
				}
			} while (ptr->is_started());
		...
		inline std::uint32_t gen_kcp_conv(std::string_view first_data) {
			std::uint32_t conv = 0;
			if (kcp::is_kcphdr_syn(first_data)) {
				...
				if (syn.th_ack == 0) {
					...
					if (conv == 0) conv = this->kcp_convs_.fetch_add(1);
				}
				else {
					conv = syn.th_ack;
				}
		...
		/**
		 * @brief Provide a kcp conv, If the conv is not provided, it will be generated by the server (gen_kcp_conv).
		 */
		inline void set_kcp_conv(std::uint32_t conv) requires is_cli_c<SvrOrCli> {
			this->kcp_conv_ = conv;
		}
		...
		asio::awaitable<error_code> kcp_handshake_t(SessionPtr&& ptr) requires is_svr_c<SvrOrCli> {
			...
			this->kcp_conv_ = gen_kcp_conv(data);
		...
		asio::awaitable<error_code> kcp_handshake_t (SessionPtr&& ptr) requires is_cli_c<SvrOrCli> {
			...
			auto executor = co_await asio::this_coro::executor;
			std::shared_ptr<asio::steady_timer> ctimer_ptr = std::make_shared<asio::steady_timer>(executor);
            asio::co_spawn(executor, [this, ptr, seq, ctimer_ptr]() -> asio::awaitable<void> { 
				while(true) {
					ctimer_ptr->expires_after(std::chrono::milliseconds(500));
					asio::error_code ec;
          			co_await ctimer_ptr->async_wait(asio::redirect_error(asio::use_awaitable, ec));
					if (ec == asio::error::operation_aborted) {
						co_return;
					}
					this->kcp_send_syn_t(seq, ec);
					if (ec) {
						if (ptr->state() == estate::started) co_await ptr->co_stop(ec);
						co_return;
					}
				}
			}, asio::detached);
		...
		inline asio::awaitable<error_code> stream_start([[maybe_unused]] SessionPtr&& ptr) {
			try {
				auto ec = co_await this->kcp_handshake_t(ptr);
				if (!ec) send_fin_ = true;
				co_return ec;
			}
	...
	protected:
		std::shared_ptr<asio::steady_timer> kcp_timer_;
