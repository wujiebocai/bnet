...
#define CHECK_POINT(p)	\
	...
	constexpr bool is_same_category = std::is_class_v<T1> && std::is_class_v<T2> && std::is_same_v<T1, T2>;
	...
	public:
		...
		inline bool call(IdxType evt, Args&&... args) {
			const auto& itr = this->func_proxy_.find(evt);
			if (itr == this->func_proxy_.end()) {
				return false;
			}

			using func_proxy_type = func_proxy<Args...>;
			func_proxy_type* fp_ptr = static_cast<func_proxy_type*>(itr->second.get());
			if (fp_ptr) {
		...
		//reference OR point
		template<class Ret, class C, class ...Args, class T>
		requires c_is_same_category<C, T>
