...
-> decltype(bnet::pointer_traits<T>::to_address(v))
{
    return bnet::pointer_traits<T>::to_address(v);
}
...
{
    return bnet::detail::ptr_address(v.operator->(), 0);
}
...
{
    return bnet::detail::ptr_address(v, 0);
}
#else
template<class T>
...
to_address(const T& v) BOOST_NOEXCEPT
{
    return bnet::to_address(v.operator->());
}
#endif
