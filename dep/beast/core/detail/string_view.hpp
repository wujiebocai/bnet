...
// forward declaration of bnet::basic_string_view from Utility
template<class Ch, class Tr> class basic_string_view;

// forward declaration of bnet::hash_range from ContainerHash
...
public:
    ...
    template<class End> BOOST_CXX14_CONSTEXPR basic_string_view( Ch const* first, End last,
        typename bnet::enable_if<bnet::core::detail::is_same<End, Ch const*> >::type* = 0 ) BOOST_NOEXCEPT: p_( first ), n_( last - first )
    {
    ...
    template<class Ch2> basic_string_view( bnet::basic_string_view<Ch2, std::char_traits<Ch2> > const& str,
        typename bnet::enable_if<bnet::core::detail::is_same<Ch, Ch2> >::type* = 0 ) BOOST_NOEXCEPT: p_( str.data() ), n_( str.size() )
    {
    }
...
# endif
    ...
    template<class Ch2, class En = typename bnet::enable_if<bnet::core::detail::is_same<Ch2, Ch> >::type>
    ...
    template<class Ch2> operator bnet::basic_string_view<Ch2,
        typename bnet::enable_if<bnet::core::detail::is_same<Ch2, Ch>, std::char_traits<Ch> >::type> () const BOOST_NOEXCEPT
    {
        return bnet::basic_string_view< Ch, std::char_traits<Ch> >( data(), size() );
    }
    ...
    BOOST_CXX14_CONSTEXPR const_reference at( size_type pos ) const
    {
        if( pos >= size() )
        {
            bnet::throw_exception( std::out_of_range( "basic_string_view::at" ), BOOST_CURRENT_LOCATION );
        }
    ...
    BOOST_CXX14_CONSTEXPR size_type copy( Ch* s, size_type n, size_type pos = 0 ) const
    {
        if( pos > size() )
        {
            bnet::throw_exception( std::out_of_range( "basic_string_view::copy" ), BOOST_CURRENT_LOCATION );
        }
    ...
    BOOST_CXX14_CONSTEXPR basic_string_view substr( size_type pos = 0, size_type n = npos ) const
    {
        if( pos > size() )
        {
            bnet::throw_exception( std::out_of_range( "basic_string_view::substr" ), BOOST_CURRENT_LOCATION );
        }
    ...
    {
        return bnet::hash_range( sv.begin(), sv.end() );
    }
};
...
struct std::basic_common_reference<
    ...
    Q1, Q2>
{
    using type = bnet::core::basic_string_view<Ch>;
};
