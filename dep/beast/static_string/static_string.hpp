...
#include <beast/static_string/config.hpp>
...
// - basic_string_view (bnet::string_view or std::string_view) when in
...
public:
  ...
  */
  ...
  BOOST_STATIC_STRING_CPP11_CONSTEXPR
  operator ::bnet::basic_string_view<CharT, Traits>() const noexcept
  {
    return ::bnet::basic_string_view<CharT, Traits>(data(), size());
  }

  BOOST_STATIC_STRING_CPP11_CONSTEXPR
  operator ::bnet::core::basic_string_view<CharT>() const noexcept
  {
    return ::bnet::core::basic_string_view<CharT>(data(), size());
  }
#endif
#endif
...
{
  return bnet::hash_range(str.begin(), str.end());
}
#endif
} // static_strings
...
struct hash<
  ...
  basic_static_string
#else
  bnet::static_strings::basic_static_string<N, CharT, Traits>
#endif
  >
{
  std::size_t
  operator()(
    const bnet::static_strings::basic_static_string<N, CharT, Traits>& str) const noexcept
  {
#if !defined(BOOST_STATIC_STRING_STANDALONE)
    return bnet::hash_range(str.begin(), str.end());
#elif defined(BOOST_STATIC_STRING_HAS_ANY_STRING_VIEW)
    using view_type = typename
      bnet::static_strings::basic_string_view<CharT, Traits>;
