...
/** Returns `true` if the specified HTTP request is a WebSocket Upgrade.
    ...
    {
        bnet::beast::flat_buffer buffer;
        bnet::beast::http::request<bnet::beast::http::string_body> req;
        bnet::beast::http::read(sock, buffer, req);
        if(bnet::beast::websocket::is_upgrade(req))
        {
            bnet::beast::websocket::stream<decltype(sock)> ws{std::move(sock)};
            ws.accept(req);
        }
    }
    @endcode
...
#include <beast/beast/websocket/impl/rfc6455.hpp>
