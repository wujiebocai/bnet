...
#include <beast/smart_ptr/weak_ptr.hpp>
...
{
    std::shared_ptr<detail::stream_service_impl> sp_;
...
{
    net::any_io_executor exec;
    std::weak_ptr<stream_service_impl> wp;
    std::mutex m;
    flat_buffer b;
    ...
    stream_state(
        ...
        std::weak_ptr<stream_service_impl> wp_,
        fail_count* fc_);
