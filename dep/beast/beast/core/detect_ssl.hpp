...
#include <beast/logic/tribool.hpp>
...
/** Return `true` if the buffer contains a TLS Protocol client_hello message.
    ...
    @li `bnet::indeterminate` if the buffer contains an
    ...
    @return `bnet::tribool` indicating whether the buffer contains
...
struct run_detect_ssl_op
{
    ...
    // signature, in this case `void(error_code, bnet::tri_bool)`.
