...
{
    static_assert(bnet::beast::is_sync_read_stream<next_layer_type>::value,
...
async_read_some(
    ...
    ReadHandler&& handler)
{
    static_assert(bnet::beast::is_async_read_stream<next_layer_type>::value,
...
{
    static_assert(bnet::beast::is_sync_write_stream<next_layer_type>::value,
...
write_some(ConstBufferSequence const& buffers, error_code& ec)
{
    ...
    return stream_.write_some(
        bnet::beast::buffers_prefix(result.size, buffers), ec);
}
...
{
    static_assert(bnet::beast::is_async_write_stream<next_layer_type>::value,
...
void
teardown(
    bnet::beast::role_type role,
    flat_stream<NextLayer>& s,
    error_code& ec)
{
    using bnet::beast::websocket::teardown;
...
{
    using bnet::beast::websocket::async_teardown;
