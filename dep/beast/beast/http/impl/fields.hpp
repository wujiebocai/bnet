...
#include <beast/beast/core/detail/buffers_ref.hpp>
#include <beast/beast/core/detail/clamp.hpp>
#include <beast/beast/core/detail/static_string.hpp>
#include <beast/beast/core/detail/temporary_buffer.hpp>
#include <beast/beast/core/static_string.hpp>
#include <beast/beast/http/verb.hpp>
#include <beast/beast/http/rfc7230.hpp>
#include <beast/beast/http/status.hpp>
#include <beast/beast/http/chunk_encode.hpp>
...
basic_fields(Allocator const& alloc) noexcept
    : bnet::empty_value<Allocator>(bnet::empty_init_t(), alloc)
{
}
...
basic_fields(basic_fields&& other) noexcept
    : bnet::empty_value<Allocator>(bnet::empty_init_t(),
        std::move(other.get()))
...
basic_fields(basic_fields const& other)
    : bnet::empty_value<Allocator>(bnet::empty_init_t(), alloc_traits::
    ...
    string_view sname, string_view const& value)
{
	...
	bnet::ignore_unused(iter, itel);
	BOOST_ASSERT(std::distance(set_.begin(), iter) == std::distance(list_.begin(), itel));
}
...
erase(const_iterator pos) ->
    const_iterator
{
	BOOST_ASSERT(std::next(set_.begin(), std::distance(list_.begin(), pos))->second->self_ == pos->self_);
    auto next = pos;
    auto& e = *next++;
    ...
    const_iterator
{
	...
	BOOST_ASSERT(std::next(list_.begin(), std::distance(set_.begin(), it))->self_ == it->second->self_);
