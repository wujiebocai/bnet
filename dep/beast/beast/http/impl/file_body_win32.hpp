...
#include <beast/beast/core/async_base.hpp>
#include <beast/beast/core/bind_handler.hpp>
#include <beast/beast/core/buffers_range.hpp>
#include <beast/beast/core/detail/clamp.hpp>
#include <beast/beast/core/detail/is_invocable.hpp>
#include <beast/beast/http/error.hpp>
#include <beast/beast/http/write.hpp>
#include <beast/beast/http/serializer.hpp>
#include <asio/async_result.hpp>
#include <asio/basic_stream_socket.hpp>
#include <asio/windows/overlapped_ptr.hpp>
#include <memory>
#include <beast/smart_ptr/make_shared_array.hpp>
#include <beast/winapi/basic_types.hpp>
...
struct basic_file_body<file_win32>
{
    ...
    public:
                ...
                error_code& ec)
        {
            ...
            bnet::ignore_unused(content_length);
            BOOST_ASSERT(body_.file_.is_open());
            ec = {};
        }
...
error_code
make_win32_error(
    bnet::winapi::DWORD_ dwError) noexcept
{
    // from
    ...
    switch(dwError)
    {
    case bnet::winapi::ERROR_NETNAME_DELETED_:
        return net::error::connection_reset;
    case bnet::winapi::ERROR_PORT_UNREACHABLE_:
        return net::error::connection_refused;
    case bnet::winapi::WSAEMSGSIZE_:
    case bnet::winapi::ERROR_MORE_DATA_:
        return {};
    }
    return error_code(
    ...
    error_code ec) noexcept
{
    ...
    return make_win32_error(
        static_cast<bnet::winapi::DWORD_>(
            ec.value()));
}
...
public:
    ...
    operator()()
    {
        ...
        bnet::winapi::DWORD_ const nNumberOfBytesToWrite =
            static_cast<bnet::winapi::DWORD_>(
            (std::min<std::uint64_t>)(
                (std::min<std::uint64_t>)(w.body_.last_ - w.pos_, sr_.limit()),
                (std::numeric_limits<bnet::winapi::INT_>::max)() - 1));
