...
#ifndef __KCP_UTIL_HPP__
#define __KCP_UTIL_HPP__
...
//	};
	...
	std::string to_string(kcphdr& hdr)
	{
		...
		tool::write(p, hdr.th_seq      );
		tool::write(p, hdr.th_ack      );
		tool::write(p, hdr.th_flag.byte);
		tool::write(p, hdr.th_padding  );
		tool::write(p, hdr.th_sum      );
	...
	kcphdr to_kcphdr(std::string_view s) noexcept
	{
		...
		{
			hdr.th_seq       = tool::read<std::uint32_t>(p);
			hdr.th_ack       = tool::read<std::uint32_t>(p);
			hdr.th_flag.byte = tool::read<std::uint8_t >(p);
			hdr.th_padding   = tool::read<std::uint8_t >(p);
			hdr.th_sum       = tool::read<std::uint16_t>(p);
		}
